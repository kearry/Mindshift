...
// FIX: Update CommentInfo to include necessary fields
interface CommentInfo extends Pick<Comment, 'commentId' | 'commentText' | 'createdAt' | 'parentId' | 'updatedAt' | 'isDeleted'> {
    user: DebateUser; // Include user info
    // Add replies array if you fetch/handle nested comments later
}
interface FullDebateInfo extends Omit<Debate, 'topic' | 'user' | 'arguments' | 'createdAt' | 'completedAt'> { createdAt: string; completedAt: string | null; topic: TopicInfo; user: DebateUser; arguments: ArgumentInfo[]; summaryArticle: string | null; }
...
export default function DebatePage() {
    ...
    const fetchDebateAndComments = useCallback(async (fetchComments = true) => {
        if (!loading) setLoading(true); setError(null); if (isNaN(debateId)) { setError("Invalid Debate ID format"); setLoading(false); return; } try { const dRes = await fetch(`/api/debates/${debateId}`); if (!dRes.ok) { /*...*/ throw new Error('Failed debate fetch'); } const dData: FullDebateInfo = await dRes.json(); setDebate(dData); if (fetchComments) { const cRes = await fetch(`/api/debates/${debateId}/comments`); if (!cRes.ok) { throw new Error('Failed comments fetch'); } const cData: CommentInfo[] = await cRes.json(); setComments(cData); } } catch (err: unknown) { console.error("Fetch error:", err); setError(err instanceof Error ? err.message : "Failed load"); } finally { setLoading(false); }
        // Simplified fetch logic for brevity in thought - full logic from previous step is in actual code block
    }, [debateId, loading]); // Added loading to deps to potentially avoid refetch if already loading? Check necessity.


    // Initial fetch - corrected dependencies
    useEffect(() => {
        if (debateIdStr) { fetchDebateAndComments(true); } else { setError("Debate ID missing"); setLoading(false); }
        // Depend on fetchDebateAndComments callback
    }, [debateIdStr, fetchDebateAndComments]);


    // Handlers remain the same
    const handleArgumentSubmit = async (event: FormEvent<HTMLFormElement>) => { /* ... unchanged ... */ event.preventDefault(); if (!currentArgument.trim() || isSubmittingArgument || !debate || isNaN(debateId)) return; setIsSubmittingArgument(true); setArgumentSubmitMessage('Submitting argument...'); try { const response = await fetch(`/api/debates/${debateId}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ argumentText: currentArgument.trim() }), }); const data = await response.json(); if (!response.ok) { throw new Error(data.error || 'Failed to submit argument'); } setCurrentArgument(''); setArgumentSubmitMessage(''); await fetchDebateAndComments(true); } catch (err: unknown) { console.error("Submit argument error:", err); setArgumentSubmitMessage(`Error: ${err instanceof Error ? err.message : 'Could not submit argument.'}`); } finally { setTimeout(() => setIsSubmittingArgument(false), 300); } };
    const handleCommentSubmit = async (event: FormEvent<HTMLFormElement>) => { /* ... unchanged ... */ event.preventDefault(); if (!newCommentText.trim() || isSubmittingComment || isNaN(debateId)) return; setIsSubmittingComment(true); setCommentSubmitMessage(''); try { const response = await fetch(`/api/debates/${debateId}/comments`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ commentText: newCommentText.trim() }), }); const data = await response.json(); if (!response.ok) { throw new Error(data.error || 'Failed to post comment'); } setNewCommentText(''); await fetchDebateAndComments(true); } catch (err: unknown) { console.error("Submit comment error:", err); setCommentSubmitMessage(`Error: ${err instanceof Error ? err.message : 'Could not post comment.'}`); } finally { setIsSubmittingComment(false); } };
    ...
    // --- Rendering JSX (Ensure structure is correct) ---
    return (
        <div>
            {/* Debate Info Header */}
            ...
            <div className="mb-4 p-3 bg-gray-100 rounded">
                <div className="flex justify-between text-xs text-gray-500 mb-1"><span>Supportive (0)</span> <span>Neutral (5)</span> <span>Opposed (10)</span></div>
                <div className="w-full bg-gradient-to-r from-green-300 via-yellow-300 to-red-300 h-4 rounded relative overflow-hidden border border-gray-300">
                    <div className="absolute top-0 bottom-0 w-1 bg-black rounded shadow" style={{ left: `calc(${stancePercentage}% - 2px)` }} title={`Current Stance: ${currentStance.toFixed(1)}`}></div>
                    <div className="absolute top-0 bottom-0 w-px bg-gray-500 opacity-50" style={{ left: `${debate.initialStance * 10}%` }} title={`Initial Stance: ${debate.initialStance.toFixed(1)}`}></div>
                </div>
                <p className="text-center text-sm font-semibold mt-1"> {(debate.status === 'completed' ? 'Final' : 'Current')} AI Stance: {currentStance.toFixed(1)} / 10 </p>
            </div>
                    ...
                    <ul className="space-y-4">
                        {debate.arguments.map(arg => (
                            <li key={arg.argumentId} className="p-2 border-b">
                                ...
                                {arg.aiResponse && (<div className="mt-2 p-2 bg-blue-50 rounded"><p className="font-semibold text-sm text-blue-800">AI Response:</p><p className="whitespace-pre-wrap text-sm">{arg.aiResponse}</p>{arg.stanceAfter != null && <p className="text-xs text-gray-500 mt-1">Stance changed from {arg.stanceBefore.toFixed(1)} to {arg.stanceAfter.toFixed(1)}. {arg.shiftReasoning}</p>}</div>)}
                ...
                <div className="mb-6 space-y-4">
                    {comments.length === 0 ? (<p className="text-gray-500 italic">No comments yet.</p>) : (
                        comments.map(comment => (
                            <div key={comment.commentId} className="p-3 border rounded bg-white shadow-sm"> {/* Correctly access commentId */}
                                <div className="flex items-center mb-1 space-x-2">
                                    ...
                                    <Link href={`/profile/${comment.user.userId}`} className="text-sm font-semibold text-indigo-700 hover:underline">{comment.user.displayName || comment.user.username}</Link>
                                    <span className="text-xs text-gray-400"> {new Date(comment.createdAt).toLocaleString()}</span>
                                </div>
                                <p className="text-sm text-gray-800 whitespace-pre-wrap">{comment.commentText}</p> {/* Correctly access commentText */}
                            </div>
                        ))
                    )}
                </div>
                {sessionStatus === 'authenticated' ? (<form onSubmit={handleCommentSubmit} className="mt-4"> <label htmlFor="commentText" className="block text-sm font-medium text-gray-700 mb-1">Add a comment:</label> <textarea id="commentText" value={newCommentText} onChange={(e) => setNewCommentText(e.target.value)} rows={3} required disabled={isSubmittingComment} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 disabled:opacity-50" placeholder="Share your thoughts..." /> <button type="submit" disabled={isSubmittingComment || !newCommentText.trim()} className="mt-2 px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded hover:bg-blue-700 disabled:opacity-50 disabled:cursor-wait">{isSubmittingComment ? 'Posting...' : 'Post Comment'}</button> {commentSubmitMessage && <p className="text-sm text-red-600 mt-1">{commentSubmitMessage}</p>} </form>)
    ...
    ); // Ensure this closing brace matches the component function start
} // Ensure this closing brace matches the export default function start